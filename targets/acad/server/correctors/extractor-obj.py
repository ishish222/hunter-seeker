...
while True:
    try:
        if(check_d_sect()):
            ...
            sect.decode()
            print("Dmping data section at: " + hex(sect.offset) + ", data at: " + hex(sect.dataOffset) + ", cSize: " + hex(sect.cSize) + ", dSize: " + hex(sect.dSize))
            fd.seek(sect.dataOffset)
#            print("Reading " + hex(sect.cSize) + " at " + hex(fd.tell()))
#            print("Like " + hex(ord(fmap[fd.tell()])))
            dirr = o + "/" + os.path.basename(f) + "-sdump"
            testdir(dirr)
            fo = open(dirr + "/" + hex(sect.offset), "w+b")
            fo.write(fd.read(sect.cSize))
            fo.close()
#            while True:
#                sect.calc_checksum()
#                sect.calc_checksum2()
