...
def prepare_disk_glob_temu():
    ...
    globs.state.options.tmp_disk_img = create_drive(options, size=options.disk_size, offset=32256, format_cmd='copy_secondary')
    mount_drive(options, 32256)
    create_layout(options)
    for sample in state.samples_list:
        print(options.tmp_mountpoint + "/samples/shared/" + sample)
        os.spawnv(os.P_WAIT, "/bin/cp", ["cp", sample, options.tmp_mountpoint + "/samples/shared/"])
...
def mount_drive(options):
    globs.state.options.tmp_mountpoint = tempfile.mktemp()
...
def create_drive(options, size=None, name=None, label=None, offset=0, format_cmd='mkfs.ntfs'):
    ...
    if(format_cmd == 'copy_secondary'):
        os.spawnv(os.P_WAIT, "/bin/cp", ["cp", options.qemu_drives_dir + "/" + qemu_secondary, name])
    else:
