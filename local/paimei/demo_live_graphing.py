...
start = time.time()
print("loading vonage.exe.pida ...", end=' ')
...
bps = [function.ea_start for function in list(vonage.nodes.values()) if not function.is_import]
print("setting breakpoints on %d of %d functions" % (len(bps), len(list(vonage.nodes.values()))))
