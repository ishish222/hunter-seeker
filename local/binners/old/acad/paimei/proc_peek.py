...
def process_pp_hit (dbg):
    ...
    if not noint and peek_points[ea].disabled != -1 and hit_count in (5, 10, 20, 50, 75, 100, 150):
        ...
        try:
            key = input("<y|n|v|q>[c] y=yes, n=no, v=never, c=comment> ")
            ...
            if key.lower() in ("yc", "nc"):
                peek_points[ea].comment += " // " + input("add comment> ")
        except:
            pass
...
def handler_breakpoint (dbg):
    ...
    if dbg.first_breakpoint:
        ...
        print("setting breakpoints on %d peek points" % len(list(peek_points.keys())))
        dbg.bp_set(list(peek_points.keys()))
    ...
    if dbg.exception_address in peek_points:
        process_pp_hit(dbg)
