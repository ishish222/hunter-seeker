...
int parse_trace_watchpoints(char* string, taint_x86* taint_eng)
{
    ...
    while((current != 0x0) && (strlen(current) > 0x0))
    {
        memset(&twp, 0x0, sizeof(TRACE_WATCHPOINT));
        strcpy(buf, current);
        if(taint_eng->parse_trace_string(buf, &twp) != 0x0) return -1;
        taint_eng->add_trace_watchpoint(twp);
        current = strpbrk(current, "+");
        if(current)
            current++;
    }
...
int main(int argc, char** argv)
{
    ...
    printf("Veryfing basic operations\n");
...
*/
    ...
    while ((opt = getopt(argc, argv, "i:d:s:e:T:l:m:b:t:w:h:p:")) != -1) 
    {
        switch (opt) 
        {
            case 'i': 
    ...
    taint_eng.open_prompt_file(prompt_log);
//    taint_eng.set_lib_dir_path(loadable_dir_path);
    ...
    if(!taint_eng.start_addr)
        taint_eng.started = 0x1;
    else
        taint_eng.started = 0x0;
