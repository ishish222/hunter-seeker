...
#define CODE_RED    0x3
    ...
    if(s)
    {
        if(s->wanted) d_err_print("[x] %s!%s\n", s->lib_name, s->func_name);
        else d_err_print("%s!%s\n", s->lib_name, s->func_name);
    }
...
//    if(!this->started || this->finished) return 0x0;
    ...
    d_print(1, "[0x%08x] 0x%08x call 0x%08x, pos: %d, small: %d, ignored: %d: \n", this->cur_tid, current.get_DWORD(), target, this->ctx_info[this->tids[this->cur_tid]].graph_pos, this->ctx_info[this->tids[this->cur_tid]].graph_pos_smallest, this->ctx_info[this->tids[this->cur_tid]].graph_pos_ignored, this->ctx_info[this->tids[this->cur_tid]].graph_pos_largest);
/*
    ...
    /* check surface */
        ...
        for(i = cur_ctx->graph_pos-1; i >= cur_ctx->graph_pos_smallest; i--)
        {
            if(abs(cur_ctx->rets[i] - eip) < 0x5)
            {
                ...
                d_print(1, "[0x%08x] Matched ret 0x%08x on pos: %d, handling diff: %d\n", this->cur_tid, cur_ctx->rets[i], i, diff);
...
int taint_x86::pre_execute_instruction(DWORD eip)
{
    ...
    if(eip == 0x77f064f0)
    d_err_print("Syscall no: 0x%08x\n", this->reg_restore_32(EAX).get_DWORD());
...
int taint_x86::a_check_segment_override()
{
    ...
    {
        ret += this->ctx_info[this->cur_tid].seg_map[SEG_GS];
        d_print(3, "GS! 0x%08x\n", this->ctx_info[this->cur_tid].seg_map[SEG_GS]);
    }
    if(this->current_prefixes & PREFIX_FS_OVERRIDE)
    {
        ret += this->ctx_info[this->cur_tid].seg_map[SEG_FS];
        d_print(3, "FS! 0x%08x\n", this->ctx_info[this->cur_tid].seg_map[SEG_FS]);
    }
    if(this->current_prefixes & PREFIX_ES_OVERRIDE)
    {
        ret += this->ctx_info[this->cur_tid].seg_map[SEG_ES];
        d_print(3, "ES! 0x%08x\n", this->ctx_info[this->cur_tid].seg_map[SEG_ES]);
    }
    if(this->current_prefixes & PREFIX_DS_OVERRIDE)
    {
        ret += this->ctx_info[this->cur_tid].seg_map[SEG_DS];
        d_print(3, "DS! 0x%08x\n", this->ctx_info[this->cur_tid].seg_map[SEG_DS]);
    }
    if(this->current_prefixes & PREFIX_CS_OVERRIDE)
    {
        ret += this->ctx_info[this->cur_tid].seg_map[SEG_CS];
        d_print(3, "CS! 0x%08x\n", this->ctx_info[this->cur_tid].seg_map[SEG_CS]);
    }
    if(this->current_prefixes & PREFIX_SS_OVERRIDE)
    {
        ret += this->ctx_info[this->cur_tid].seg_map[SEG_SS];
        d_print(3, "SS! 0x%08x\n", this->ctx_info[this->cur_tid].seg_map[SEG_SS]);
    }
