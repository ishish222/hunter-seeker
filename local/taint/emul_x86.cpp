...
void taint_x86::reg_store_32_correct(OFFSET off, DWORD_t v, int tid)
{
    if(this->options & OPTION_VERIFY_OOB)
        if(this->verify_oob_offset(off, REG_SIZE) != 0x0) 
            return;
    ...
    info->registers[off + 0].set_BYTE(v[0].get_BYTE());
    info->registers[off + 1].set_BYTE(v[1].get_BYTE());
    info->registers[off + 2].set_BYTE(v[2].get_BYTE());
    info->registers[off + 3].set_BYTE(v[3].get_BYTE());
...
int taint_x86::mod_thread(CONTEXT_OUT ctx_out)
{
    ...
    this->reg_store_32_correct(EAX, ctx_out.ctx.Eax, ctx_out.thread_id);
    this->reg_store_32_correct(ECX, ctx_out.ctx.Ecx, ctx_out.thread_id);
    this->reg_store_32_correct(EDX, ctx_out.ctx.Edx, ctx_out.thread_id);
    this->reg_store_32_correct(EBX, ctx_out.ctx.Ebx, ctx_out.thread_id);
    this->reg_store_32_correct(ESI, ctx_out.ctx.Esi, ctx_out.thread_id);
    this->reg_store_32_correct(EDI, ctx_out.ctx.Edi, ctx_out.thread_id);
    this->reg_store_32_correct(ESP, ctx_out.ctx.Esp, ctx_out.thread_id);
    this->reg_store_32_correct(EBP, ctx_out.ctx.Ebp, ctx_out.thread_id);
    this->reg_store_32_correct(EFLAGS, ctx_out.ctx.EFlags, ctx_out.thread_id);
...
int taint_x86::clear_current_propagation_causes()
{
    ...
    current_propagation->taint_propagation = 0x0;
        ...
        case (MODRM_MEM):
            switch(rm.size)
            {
                case MODRM_SIZE_16:
                    ...
                    this->attach_current_propagation_m_16(r.offset);
                    break;
                case MODRM_SIZE_32:
                    this->restore_32(rm.offset, src_32);
                    this->reg_propagation_cause_m_32(rm.offset);
                    ...
                    this->attach_current_propagation_m_32(r.offset);
                    break;
            }
            break;
    }
...
int taint_x86::r_neg_rm_8(BYTE_t* instr_ptr)
{
    ...
    switch(rm.region)
    {
        case (MODRM_REG):
            dst_8 = this->reg_restore_8(rm.offset);
            reg_propagation_cause_r_8(rm.offset);

            dst_8.set_BYTE(~dst_8.get_BYTE()+1);

            this->reg_store_8(rm.offset, dst_8);
            attach_current_propagation_r_8(rm.offset);
            break;
        case (MODRM_MEM):
            this->restore_8(rm.offset, dst_8);
            reg_propagation_cause_r_8(rm.offset);
        ...
        case (MODRM_REG):
            switch(rm.size)
            {
                case MODRM_SIZE_16:
                    ...
                    dst_16.set_WORD(~dst_16.get_WORD()+1);

                    this->reg_store_16(rm.offset, dst_16);
                    attach_current_propagation_r_16(rm.offset);
                    break;
                case MODRM_SIZE_32:
                    dst_32 = this->reg_restore_32(rm.offset);
                    reg_propagation_cause_r_32(rm.offset);

                    dst_32.set_DWORD(~dst_32.get_DWORD()+1);

                    this->reg_store_32(rm.offset, dst_32);
                    attach_current_propagation_r_32(rm.offset);
                    break;
            }
            break;
        case (MODRM_MEM):
            switch(rm.size)
            {
                case MODRM_SIZE_16:
                    ...
                    this->store_16(rm.offset, dst_16);
                    attach_current_propagation_m_16(rm.offset);
                    break;
                case MODRM_SIZE_32:
                    this->restore_32(rm.offset, dst_32);
                    reg_propagation_cause_m_32(rm.offset);
                    ...
                    this->store_32(rm.offset, dst_32);
                    attach_current_propagation_m_32(rm.offset);
                    break;
            }
            break;
    }
...
int taint_x86::r_push_rm_16_32(BYTE_t* instr_ptr)
{
    ...
    a_push_32(temp); /* takes care of attaching propagation */
