...
#include <fstream>
#include "distorm_inc/distorm.h"

#define MEM_ALLOC_DECLARED_SIZE
#define ANALYSIS_INDEX_TAINTS
//#define VERIFY_OOB
...
int taint_x86::d_print_prompt(int level, const char* format, ...)
{
    va_list argptr;

    va_start(argptr, format);
    if(this->prompt_file)
        vfprintf(this->prompt_file, format, argptr);
    va_end(argptr);
...
int taint_x86::check_func_wanted(char* name)
{
    ...
    for(i = 0x0; i < 0x50; i++)
    {
        if(!strcmpi(name, this->func_wanted[i]))
        {
...
int taint_x86::update_watchpoints(DWORD tid)
{
    ...
    for(i=0x0; i< this->new_wpt_count; i++)
        ...
        {
            this->trace_watchpoint_connect(&this->new_wps[i]);
        }
...
//    d_err_print("Registered ptr: 0x%08x\n", this->new_wps[this->new_wpt_count].watched);
...
SYMBOL* taint_x86::get_symbol(OFFSET addr)
{
    ...
    do
    {
        if((s->addr == addr) && (s->resolved)) return s; 
        else s = s->next;
    } while(s);
...
LIB_INFO* taint_x86::get_lib(OFFSET offset)
{
    ...
    {
        d_print(2, "Checking lib: %s, 0x%08x-0x%08x\n", this->libs[i].name, this->libs[i].offset, this->libs[i].offset+this->libs[i].length);
        if(this->libs[i].offset < offset)
            if((this->libs[i].offset + this->libs[i].length) > offset)
            {
                d_print(2, "Match\n");
                return &this->libs[i];
            }
...
void print_ret(FILE* f)
{
    ...
    sprintf(out_line, "</node>\n");
    fwrite(out_line, strlen(out_line), 0x1, f);
}

int taint_x86::handle_call(CONTEXT_INFO* info, OFFSET target, OFFSET next)
{
    return 0x0;
    SYMBOL* s;
    char out_line[MAX_NAME];
...
#define CODE_RED    0x3
    ...
    if(this->cur_info->waiting != 0x0)
    {
        if((s != 0x0) && (s->wanted))
        {
            d_print(1, "Got wanted!\n");
            sprintf(out_line, "...");
            print_call(this->cur_info->graph_file, out_line, colors[CODE_RED]);
            sprintf(out_line, "0x%08x call %s!%s", current.get_DWORD(), s->lib_name, s->func_name);
            print_call(this->cur_info->graph_file, out_line, colors[CODE_RED]);
            print_ret(this->cur_info->graph_file);
            print_ret(this->cur_info->graph_file);
            return 0x0;
        }
        else
    ...
    {
        sprintf(out_line, "0x%08x call 0x%08x", current.get_DWORD(), target);
        color = CODE_BLACK;
    }

    int i;
    for(i=0x0; i< this->cur_info->graph_pos; i++)
        d_print(1, " ");

    d_print(1, "0x%08x call 0x%08x, pos: %d, small: %d, ignored: %d: ", current.get_DWORD(), target, this->cur_info->graph_pos, this->cur_info->graph_pos_smallest, this->cur_info->graph_pos_ignored, this->cur_info->graph_pos_largest);


    if(this->check_addr_blacklist(target))
    {
        this->cur_info->waiting = next;
        d_print(1, "Blacklisted, waiting for 0x%08x\n", next);
        print_empty_call(this->cur_info->graph_file, out_line, colors[color]);
        this->cur_info->waiting = next;
    }
    else if(this->cur_info->graph_pos > this->depth)
    {
        this->cur_info->waiting = next;
        d_print(1, "Reached max, waiting for 0x%08x\n", next);
    ...
    {
        d_print(1, "Writing out\n");
        print_call(this->cur_info->graph_file, out_line, colors[color]);
        this->cur_info->graph_pos ++;
        this->rets[this->ret_idx] = next;
        d_print(1, "Push: 0x%08x\n", next);
        this->ret_idx ++; 
    }
    //getchar();
    //d_print(1, "pos: %d\n", this->cur_info->graph_pos);
...
int taint_x86::handle_ret(CONTEXT_INFO* info)
{
    return 0x0;
    ...
    idx = this->check_rets(ret.get_DWORD());

    d_print(1, "Ret: 0x%08x\n", ret.get_DWORD());

    if(idx == -1) 
        return 0x0;
    else
    {
        d_print(1, "Received ret: 0x%08x @ 0x%02x\n", this->rets[idx], idx);
        this->ret_idx = idx;
    }
/*
    if(!this->cur_info->waiting && this->rets[this->cur_info->graph_pos] != ret.get_DWORD()) 
    {
        d_print(1, "Ingoring ret, because: 0x%08x != 0x%08x\n", this->rets[this->cur_info->graph_pos], ret.get_DWORD());
        return 0x0;
    }
*/
    if(this->cur_info->graph_pos < 0)
    {
        d_print(1, "Detected call/ret desynchronisation, aborting.\n");
        d_print(1, "Please try to add library to the blacklist or analyze with smaller step\n");
        this->started = 0x0;
        this->finished = 0x1;
        this->aborted = 0x1;
        return 0x0;
    ...
    d_print(2, "Ret:  0x%08x, started: %x, waiting: %x\n", this->reg_restore_32(EIP).get_DWORD(), this->started, this->cur_info->waiting);
    /* do we handle? */
    if(!this->started || this->finished) return 0x0;
    if(this->cur_info->waiting) return 0x0;

    int i;
    for(i=0x0; i< this->cur_info->graph_pos; i++)
        d_print(1, " ");

    d_print(1, "ret, pos: %d, small: %d, ignored: %d: ", this->cur_info->graph_pos, this->cur_info->graph_pos_smallest, this->cur_info->graph_pos_ignored);
    if(this->cur_info->graph_pos <0) return 0x0;

    d_print(1, "Writing out\n");
    print_ret(this->cur_info->graph_file);
    if(this->cur_info->graph_pos < this->cur_info->graph_pos_smallest) this->cur_info->graph_pos_smallest = this->cur_info->graph_pos;
    this->cur_info->graph_pos_ignored--;
    this->cur_info->graph_pos --;

    if(this->cur_info->graph_pos_ignored <0) this->cur_info->graph_pos_ignored = 0;
    //if(this->cur_info->graph_pos <0) this->cur_info->graph_pos = 0;
...
int taint_x86::pre_execute_instruction(DWORD eip)
{
    ...
    //d_print(1, "Waiting: 0x%08x, eip: 0x%08x\n", this->cur_info->waiting, eip);
    if(this->cur_info->waiting == eip) 
    {
        d_print(2, ", Got waited, 0x%08x\n", eip);
        this->cur_info->waiting = 0x0;
    }
    ...
    /* test if propagation is clear */
    if(this->propagations[this->current_propagation_count].causes != 0x0)
    {
        d_print(3, "Error: previous propagation has not been attached, causes: 0x%08x\n", this->propagations[this->current_propagation_count].causes);
//        this->propagations[this->current_propagation_count].causes = 0x0;
//        this->propagations[this->current_propagation_count].first_op = 0x0;
    }

    /* prepare propagation */
    /* deprecated */
    /*if(this->current_propagation == 0x0)
    {
        this->current_propagation = (PROPAGATION*)malloc(sizeof(PROPAGATION));
    }*/
...
int taint_x86::post_execute_instruction(DWORD eip)
{
    ...
    if(this->current_last > MAX_LAST_INSTRUCTIONS-1) current_last = 0x0;
    this->last_instructions[this->current_last] = eip;
    this->current_last++;
    ...
    {
        if((this->start_addr < eip) && (this->start_addr + 0x10 > eip)) 
        {
            this->finished = 0x1;
            d_print(1, "Eip: 0x%08x, this->start+0x10: 0x%08x, finishing\n", eip, this->start_addr + 0x10);
        }
    }
    fflush(stdout);
    ...
    for(i=0x0; i<this->new_wpt_count; i++)
        if((this->new_wps[i].offset == this->current_instr_count) && (this->new_wps[i].offset))
        {
            ...
            this->print_taint_history(this->new_wps[i].watched, this->new_wps[i].branches);
...
int taint_x86::check_thread(CONTEXT_info ctx_info)
{
    ...
    error:
        if(last_inconsistent == 0x0)
            d_print(3, "WARNING: New inconsistency detected @ %d, eip: 0x%08x\n", this->current_instr_count, this->current_eip);
        else
            d_print(3, "WARNING: inconsistency detected\n");
...
int taint_x86::add_thread(CONTEXT_info ctx_info)
{
    ...
    /*
        ...
        d_print(3, "Creating graph file: %s\n", graph_filename);
        ...
        this->ctx_info[this->tid_count].graph_pos = 0x0; //for call trace
    */
...
int taint_x86::add_symbols(LIB_INFO* info)
{
    ...
    do
    {
        ...
        {
            d_print(3, "Resolved symbol: %s!%s\n", s->lib_name, s->func_name);
            s->addr += info->offset;
            s->resolved = 0x1;
        }
        s = s->next;
/*
        {
            //this->copy_symbol(&info->symbols, s);

            if(info->symbols == 0x0) //dst list empty
            {
                info->symbols = s;
                old->next = s->next;
                s->next = 0x0;
            ...
            s->addr += info->offset;
            d_print(1, ", copied symbol %s @ 0x%08x + 0x%08x\n", s->func_name, info->offset, s->addr);
        } 
        else
        {
            d_print(1, "Not matched: %s and %s\n", info->name, s->lib_name);
            old = s;
            s = s->next;
        }
*/
...
int taint_x86::add_lib(OFFSET off, char* name)
{
    ...
    d_print(2, "Loading lib: %s\n", new_lib.path);

    /* loading content */
/*
    f = fopen(new_lib.path, "r");

    fseek(f, 0x0, SEEK_END);
    new_lib.length = ftell(f);
    fseek(f, 0x0, SEEK_SET);

    new_lib.offset = off;
    new_lib.content = (char*)malloc(new_lib.length);

    fread(new_lib.content, new_lib.length, 0x1, f);

    fclose(f);
*/    
    /* analyze, extract symbols */ 

    //[TODO:]

    /* apply to memory at offset */

    /* lol you need to load sections dummy */

//    this->apply_lib_layout(&new_lib);
//    free(new_lib.content);
...
int taint_x86::load_mem_from_file(char* path)
{
    ...
    while(read_bytes != size)
    {
        ...
        d_print(1, "Reading chunk: 0x%x - 0x%x\n", read_bytes, read_bytes + read);
...
int taint_x86::propagate_taint()
{
    unsigned i,j, count;
    TAINTED* cur_op;
    PROPAGATION* cur_prop;
    BYTE cur_taint, old_taint;

    cur_prop = &this->propagations[this->current_propagation_count];
    count = cur_prop->elem_count;
    cur_op = cur_prop->first_op;

    if(count == 0x0) return 0x0;
    if(cur_op == 0x0) return 0x0;

    for(i=0x0; i<0x4; i++)
    {
        for(j=0x0; j<count; j++)
        {
            if(cur_prop->result[i] != 0x0)
            {
                cur_taint = cur_op->tainted->get_BYTE_t();
                old_taint = cur_prop->result[i]->get_BYTE_t();
                cur_prop->result[i]->set_BYTE_t(old_taint | cur_taint);
            }
            cur_op = cur_op->next;
        }
...
int taint_x86::print_taint_history(unsigned id, unsigned branches)
{
...
//    d_print(3, "1\n");
    ...
    if(cur_elem)
    {
        ...
        if(branches == 0x0) count = this->propagations[id].elem_count;
        else count = branches;
        ...
        for(i=0x0; i<count;i++)
        {
            if(this->propagations[cur_elem->cause_id].instr_count != last_instr)
            {
                d_print(4, "%d -> %d\n", id, cur_elem->cause_id);
                this->print_taint_history(cur_elem->cause_id, branches);
                last_instr = this->propagations[cur_elem->cause_id].instr_count;
            }
            if(cur_elem->next)
            {
    ...
    int k;
    for(k=0x0; k<0x20; k++)
        buf[k] = this->memory[this->propagations[id].instruction +k].get_BYTE();
    ...
    res = distorm_decode(this->propagations[id].instruction, (const unsigned char*)buf, 0x20, Decode32Bits, decodedInstructions, 0x1, &decoded);

    d_print_prompt(1, "%d: (%d)0x%08x: %s%s%s\n", id, this->propagations[id].instr_count, this->propagations[id].instruction, (char*)decodedInstructions[0].mnemonic.p, decodedInstructions[0].operands.length != 0 ? " " : "", (char*)decodedInstructions[0].operands.p);
#else
...
{
    return this->print_taint_history(target, 1);
}
*/
int taint_x86::print_taint_history(BYTE_t* target, unsigned branches)
{
    unsigned taint_id;
...
{
    return this->print_taint_history(target, instr_count, 1);
}
*/
int taint_x86::print_taint_history(BYTE_t* target, OFFSET instr_count, unsigned branches)
{
...
int taint_x86::r_decode_execute_ff(BYTE_t* addr)
{
    ...
    switch(r.offset)
    {
        ...
        case EBX: //0x3 
            ...
            break;
        case ESP: //0x4
            return this->r_noop_un(addr); //jmp
            break;
        case ESI: //0x6 
...
int taint_x86::print_err_context(int tid)
{
    ...
    {
        d_print_prompt(1, "No such context: 0x%08x\n", tid);
        return 0x0;
    }
    d_print_prompt(1, "\nWARNING!!! EIP is one instruction late!\n");
    d_print_prompt(1, "Context 0x%08x:\n", tid);
    d_print_prompt(1, "EAX: 0x%08x\n", this->reg_restore_32(EAX, tid).get_DWORD());
    d_print_prompt(1, "EBX: 0x%08x\n", this->reg_restore_32(EBX, tid).get_DWORD());
    d_print_prompt(1, "ECX: 0x%08x\n", this->reg_restore_32(ECX, tid).get_DWORD());
    d_print_prompt(1, "EDX: 0x%08x\n", this->reg_restore_32(EDX, tid).get_DWORD());
    d_print_prompt(1, "ESI: 0x%08x\n", this->reg_restore_32(ESI, tid).get_DWORD());
    d_print_prompt(1, "EDI: 0x%08x\n", this->reg_restore_32(EDI, tid).get_DWORD());
    d_print_prompt(1, "EBP: 0x%08x\n", this->reg_restore_32(EBP, tid).get_DWORD());
    d_print_prompt(1, "ESP: 0x%08x\n", this->reg_restore_32(ESP, tid).get_DWORD());
    d_print_prompt(1, "EIP: 0x%08x\n", this->reg_restore_32(EIP, tid).get_DWORD());
    d_print_prompt(1, "EFL: 0x%08x\n", this->reg_restore_32(EFLAGS, tid).get_DWORD());
    d_print_prompt(1, "\n");

    DWORD eflags;
    eflags = this->reg_restore_32(EFLAGS).get_DWORD();
    if(eflags & EFLAGS_CF) d_print_prompt(1, "CF\n");
    if(eflags & EFLAGS_PF) d_print_prompt(1, "PF\n");
    if(eflags & EFLAGS_AF) d_print_prompt(1, "AF\n");
    if(eflags & EFLAGS_ZF) d_print_prompt(1, "ZF\n");
    if(eflags & EFLAGS_SF) d_print_prompt(1, "SF\n");
    if(eflags & EFLAGS_IF) d_print_prompt(1, "IF\n");
    if(eflags & EFLAGS_DF) d_print_prompt(1, "DF\n");
    if(eflags & EFLAGS_OF) d_print_prompt(1, "OF\n");
    d_print_prompt(1, "\n");
}
...
int taint_x86::print_err_t_context(int tid)
{
    ...
    d_print_prompt(1, "Taint context 0x%08x:\n", tid);
...
int taint_x86::dump_cmd(char* string)
{
    ...
    if(string[0] == 'd' && string[1] == 'l' && string[2] == 'i')
    {
        ...
        current = this->current_last;
        for(i = 0x0; i< count; i++)
        {
            d_print_prompt(1, "0x%08x\n", this->last_instructions[current]);
            current --;
...
int taint_x86::prompt_taint()
{
    ...
    while(1)
    {
        ...
        if(this->prompt_file) fprintf(this->prompt_file, "%s\n", command);
        if(command[0] == 'd')
        {
...
int taint_x86::parse_trace_string(char* string, TRACE_WATCHPOINT* twp)
{
    DWORD ctx_index;
...
int taint_x86::trace_watchpoint_connect(TRACE_WATCHPOINT* twp)
{
